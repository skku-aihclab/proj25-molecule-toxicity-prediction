...
test_loader = DataLoader(
    GraphSMILESImageDataset(g_test, s_test, i_test),
    batch_size=64,
    shuffle=False,
    collate_fn=collate
)
...
# ─── 4) hyperparameters ──────────────────────────────────────
with open(os.path.join(ROOT, "checkpoints", "parameters", "gph_smi_img_best_params.json")) as f:
    gph_smi_img_best_params = json.load(f)

emb_dim = gph_smi_img_best_params["emb_dim"]
hidden_dim = gph_smi_img_best_params["hidden_dim"]
num_layers = gph_smi_img_best_params["num_layers"]
dropout = gph_smi_img_best_params["dropout"]
epoch      = gph_smi_img_best_params["best_epoch"]
...
model.load_state_dict(torch.load(
    os.path.join(ROOT, "checkpoints", "model", "gph_smi_img.pth"), map_location=device))
...
with torch.no_grad():
    for g, t_ids, t_mask, img, y in test_loader:
        g, t_ids, t_mask, img = g.to(device), t_ids.to(device), t_mask.to(device), img.to(device)
        logits, attn_weights = model(g, t_ids, t_mask, img, return_attention=True)
...
modality_names = ['Graph', 'SMILES', 'Image']
...
plot_cross_modal_attention_heatmap(
    ...
    model_name="gph_smi_img",
    title='Graph-SMILES-Image Cross-Modal Attention',
    figsize=(10, 8)
)
