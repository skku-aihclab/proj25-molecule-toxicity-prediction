...
def objective(trial):
    ...
    train_loader = DataLoader(
        SpectrumImageDataset(sp_tr, i_tr),
        batch_size=32,
        shuffle=True,
        pin_memory=False,
        collate_fn=collate,
    ...
    valid_loader   = DataLoader(
        SpectrumImageDataset(sp_va, i_va),
        batch_size=64,
        shuffle=False,
        pin_memory=False,
        collate_fn=collate,
    ...
    for epoch in range(1,31):
        # --- train ---
        model.train()
        for ppm_list, img, y in train_loader:
        ...
        all_y, all_p = [], []
        with torch.no_grad():
            for ppm_list, img, y in valid_loader:
                img = img.to(device)
                probs = torch.sigmoid(model(ppm_list, img))
...
with open(os.path.join(ROOT, "checkpoints", "parameters", "spec_img_best_params.json"), "w") as f:
...
train_loader = DataLoader(
    SpectrumImageDataset(sp_full, i_full),
    batch_size=32,
    shuffle=True,
    pin_memory=False,
    collate_fn=collate,
...
torch.save(
    model.state_dict(),
    os.path.join(ROOT, "checkpoints", "model", "spec_img.pth")
