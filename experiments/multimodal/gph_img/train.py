...
for net in (graph_encoder, image_encoder):
...
def objective(trial):
    ...
    for epoch in range(1, 31):
        # --- train ---
        model.train()
        for g, img, y in train_loader:
            g, img, y = g.to(device), img.to(device), y.to(device)
            logits = model(g, img)      
        ...
        all_y, all_p = [], []
        with torch.no_grad():
            for g, img, y in valid_loader:
                g, img = g.to(device), img.to(device)
                probs = torch.sigmoid(model(g, img))
...
with open(os.path.join(ROOT, "checkpoints", "parameters", "gph_img_best_params.json"), "w") as f:
...
train_loader = DataLoader(
    GraphImageDataset(g_full, i_full), 
    batch_size= 32,
    shuffle= True, 
    pin_memory=False, 
    collate_fn=collate,
...
torch.save(
    model.state_dict(),
    os.path.join(ROOT, "checkpoints", "model", "gph_img.pth")
)