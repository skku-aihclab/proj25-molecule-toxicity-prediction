...
def objective(trial):
    ...
    for epoch in range(1,31):
        # --- train ---
        model.train()
        for t_ids, t_mask, img, y in train_loader:
            t_ids, t_mask, img, y = t_ids.to(device), t_mask.to(device), img.to(device), y.to(device)
        ...
        all_y, all_p = [], []
        with torch.no_grad():
            for t_ids, t_mask, img, y in valid_loader:
                t_ids, t_mask, img = t_ids.to(device), t_mask.to(device), img.to(device)
                probs = torch.sigmoid(model(t_ids, t_mask, img))
...
with open(os.path.join(ROOT, "checkpoints", "parameters", "smi_img_best_params.json"), "w") as f:
...
train_loader = DataLoader(
    SMILESImageDataset(s_full, i_full),
    batch_size=32,
    shuffle=True,
    pin_memory=False,
    collate_fn=collate,
...
torch.save(
    model.state_dict(),
    os.path.join(ROOT, "checkpoints", "model", "smi_img.pth")
