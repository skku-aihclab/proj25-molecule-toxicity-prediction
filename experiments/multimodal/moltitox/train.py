...
# ─── 4) Optuna objective ────────────────────────────────────────────────
start_search_time = time.time() # Record the start time for performance measurement
def objective(trial):
    ...
    train_loader = DataLoader(
        MoltiToxDataset(g_tr, s_tr, i_tr, sp_tr), 
        batch_size = 32, 
        shuffle= True, 
        pin_memory=False, 
        collate_fn=collate,
    ...
    valid_loader = DataLoader(
        MoltiToxDataset(g_va, s_va, i_va, sp_va),
        batch_size= 64,
        shuffle=False,
        pin_memory=False,
        collate_fn=collate,
    ...
    for epoch in range(1, 31):
        # --- train ---
        model.train()
        for g, t_ids, t_mask, img, ppm, y in train_loader:
            ...
            logits = model(g, t_ids, t_mask, img, ppm)
        ...
        all_y, all_p = [], []
        with torch.no_grad():
            for g, t_ids, t_mask, img, ppm, y in valid_loader:
                g, t_ids, t_mask, img = (
                    g.to(device),
                    t_ids.to(device),
                    t_mask.to(device),
                ...
                probs = torch.sigmoid(model(g, t_ids, t_mask, img, ppm))
...
with open(os.path.join(ROOT, "checkpoints", "parameters", "moltitox_best_params.json"), "w") as f:
...
train_loader = DataLoader(
    MoltiToxDataset(g_full, s_full, i_full, sp_full), 
    batch_size = 32, 
    shuffle= True, 
    pin_memory=False, 
    collate_fn=collate,
...
# load the encoders trained with train and valid datasets
...
torch.save(
    model.state_dict(),
    os.path.join(ROOT, "checkpoints", "model", "moltitox.pth")
)
