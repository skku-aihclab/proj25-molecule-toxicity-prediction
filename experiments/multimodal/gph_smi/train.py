...
def objective(trial):
    ...
    for epoch in range(1,31):
        # --- train ---
        model.train()
        for g, t_ids, t_mask, y in train_loader:
            g, t_ids, t_mask, y = g.to(device), t_ids.to(device), t_mask.to(device), y.to(device)
            logits = model(g, t_ids, t_mask)   
        ...
        all_y, all_p = [], []
        with torch.no_grad():
            for g, t_ids, t_mask, y in valid_loader:
                g, t_ids, t_mask = g.to(device), t_ids.to(device), t_mask.to(device)
                probs = torch.sigmoid(model(g, t_ids, t_mask))
...
with open(os.path.join(ROOT, "checkpoints", "parameters", "gph_smi_best_params.json"), "w") as f:
...
train_loader = DataLoader(
    GraphSMILESDataset(g_full, s_full),
    batch_size=32,
    shuffle=True,
    pin_memory=False,
    collate_fn=collate,
...
torch.save(
    model.state_dict(),
    os.path.join(ROOT, "checkpoints", "model", "gph_smi.pth")
